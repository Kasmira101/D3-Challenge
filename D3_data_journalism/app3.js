...
d3.csv("data.csv").then(data => {
    ...
    const y = d3.scaleLinear()
        .domain([0, d3.max(data.map(d => parseFloat(d.smokes)))])
        .range([chartHeight, 0]);

    const x = d3.scaleLinear()
        .domain([d3.min(data.map(d => parseFloat(d.age))) + - 1, d3.max(data.map(d => parseFloat(d.age)))])
        .range([0, chartWidth]);
    ...
    circleG = plotArea.selectAll("g")
        ...
        .enter()
        .append("g")
        .attr("transform", d => `translate(${x(parseFloat(d.age))}, ${y(parseFloat(d.smokes))})`)
